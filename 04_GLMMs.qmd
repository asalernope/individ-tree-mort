...
```{r}
all_spec_re <- glmmTMB(mort ~ scale(poly(dbh_mm,2))+(NCI_negexp)+scale(WD_mm_mean_ann_int)+stratum+
                          +offset(log(census_yrs))  + (1|species) + 
                            (1|plotid),
               family = binomial(link = "cloglog"),
               data = trees_new)

all_spec_fe <- glmmTMB(mort ~ scale(poly(dbh_mm,2))+stratum+(NCI_negexp)+scale(WD_mm_mean_ann_int)
...
fagus_mod_noplot <- glmmTMB(
  mort ~ scale(poly(dbh_mm, 2)) + NCI_negexp + scale(WD_mm_mean_seas_int) +
    offset(log(census_yrs)),
  family = binomial(link = "cloglog"),
  data = trees_FAGUS_up
)
...
fagus_mod_null <- glmmTMB(mort ~ scale(poly(dbh_mm,2))+(NCI_negexp)+offset(log(census_yrs))  +(1|plotid),
               family = binomial(link = "cloglog"),
               data = trees_FAGUS_up)

#vif(fagus_mod_null)

fagus_mod_clim <- glmmTMB(mort ~ scale(poly(dbh_mm,2))+(NCI_negexp)+scale(WD_mm_mean_ann_int)+
                          +offset(log(census_yrs))  +
                            (1|plotid),
               family = binomial(link = "cloglog"),
               data = trees_FAGUS_up)

fagus_mod_clim_site <- glmmTMB(mort ~ scale(poly(dbh_mm,2))+(NCI_norm)+scale(size_ratio) +  
                                 scale(WD_mm_mean_ann_int) + 
                          +offset(log(census_yrs))  +
                            (1|plotid),
               family = binomial(link = "cloglog"),
